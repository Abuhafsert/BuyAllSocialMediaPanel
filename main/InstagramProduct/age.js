import { twoYear } from "./yearsFormat/twoYears.js";
import { oneYear } from "./yearsFormat/oneYear.js";
import { threeYear } from "./yearsFormat/threeYear.js";
import { fourYear } from "./yearsFormat/fourYears.js";
import { fiveYear } from "./yearsFormat/fiveYear.js";
import { sixYear } from "./yearsFormat/sixYears.js";
import { sevenYear } from "./yearsFormat/sevenYears.js";
import { eightYear } from "./yearsFormat/eightYears.js";
import { nineYear } from "./yearsFormat/nineYears.js";
import { tenYear } from "./yearsFormat/tenYears.js";
import { sleep, dayTime } from "../AllSocialPanel/dayTime.js";
import { oneYearInstagramPriceNoPost,
twoYearInstagramPriceNoPost,
threeYearInstagramPriceNoPost,
fourYearInstagramPriceNoPost,
fiveYearInstagramPriceNoPost,
sixYearInstagramPriceNoPost,
sevenYearInstagramPriceNoPost,
eightYearInstagramPriceNoPost,
nineYearInstagramPriceNoPost,
tenYearInstagramPriceNoPost
...
const accountPrices = (uid, get, child, stt, update, ref, database)=>{
    btns.forEach(btn => {
...
function quantity(get, child, stt){
    let array = []
    
    get(child(stt, 'ageAccounts/' + 2023)).then( async (snapshot) => {
        if(snapshot.exists()) {         
    ...
    get(child(stt, 'ageAccounts/' + 2022)).then( async (snapshot) => {
        if(snapshot.exists()) {         
    ...
    get(child(stt, 'ageAccounts/' + 2021)).then( async (snapshot) => {
        if(snapshot.exists()) {         
    ...
    get(child(stt, 'ageAccounts/' + 2020)).then( async (snapshot) => {
        if(snapshot.exists()) {         
    ...
    get(child(stt, 'ageAccounts/' + 2019)).then( async (snapshot) => {
        if(snapshot.exists()) {         
    ...
    get(child(stt, 'ageAccounts/' + 2018)).then( async (snapshot) => {
        if(snapshot.exists()) {         
    ...
    get(child(stt, 'ageAccounts/' + 2017)).then( async (snapshot) => {
        if(snapshot.exists()) {         
    ...
    get(child(stt, 'ageAccounts/' + 2016)).then( async (snapshot) => {
        if(snapshot.exists()) {         
    ...
    get(child(stt, 'ageAccounts/' + 2015)).then( async (snapshot) => {
        if(snapshot.exists()) {         
    ...
    get(child(stt, 'ageAccounts/' + 2014)).then( async (snapshot) => {
        if(snapshot.exists()) {         
...
export function ageProduct(uid, get, child, stt, update, ref, database, nextSibling, input, btnValue){
    let idNo = null;
    if(btnValue === 1){
        oneYear(uid, get, child, stt, update, ref, database, nextSibling, input, idNo, current, dayTime, location, oneYearInstagramPriceNoPost)
    } else if(btnValue === 2){
        twoYear(uid, get, child, stt, update, ref, database, nextSibling, input, idNo, current, dayTime, location, twoYearInstagramPriceNoPost)
    }else if(btnValue === 3){           
        threeYear(uid, get, child, stt, update, ref, database, nextSibling, input, idNo, current, dayTime,location, threeYearInstagramPriceNoPost)
    }else if(btnValue === 4){            
        fourYear(uid, get, child, stt, update, ref, database, nextSibling, input, idNo, current, dayTime,location, fourYearInstagramPriceNoPost)
    }else if(btnValue === 5){
        fiveYear(uid, get, child, stt, update, ref, database, nextSibling, input, idNo, current, dayTime,location, fiveYearInstagramPriceNoPost)
    }else if(btnValue === 6){        
        sixYear(uid, get, child, stt, update, ref, database, nextSibling, input, idNo, current, dayTime,location, sixYearInstagramPriceNoPost)
    }else if(btnValue === 7){
        sevenYear(uid, get, child, stt, update, ref, database, nextSibling, input, idNo, current, dayTime,location, sevenYearInstagramPriceNoPost)
    }else if(btnValue === 8){
        eightYear(uid, get, child, stt, update, ref, database, nextSibling, input, idNo, current, dayTime,location, eightYearInstagramPriceNoPost)         
    }else if(btnValue === 9){
        nineYear(uid, get, child, stt, update, ref, database, nextSibling, input, idNo, current, dayTime,location, nineYearInstagramPriceNoPost)           
    }else if(btnValue === 10){
        tenYear(uid, get, child, stt, update, ref, database, nextSibling, input, idNo, current, dayTime,location, tenYearInstagramPriceNoPost)
    }
...
// const twoYear = (uid, get, child, stt, update, ref, database, nextSibling, input, idNo)=>{
