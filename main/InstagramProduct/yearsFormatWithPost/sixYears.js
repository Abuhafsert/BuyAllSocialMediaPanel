...
const sixYear = (uid, get, child, stt, update, ref, database, nextSibling, input, idNo, current, da, fileLocation, price)=>{
            ...
            get(child(stt, 'users/' + uid)).then( async (snapshot) => {
              ...
              get(child(stt, 'ageAccountsWithPost/' + 2018)).then( async (snapshot) => {
                ...
                if(snapshot.exists()) {
                  ...
                  await update(ref(database, 'ageAccountsWithPost/' + 2018 + '/'),{
                      sixYear: usd
                  ...
                  for(let l of selectt){
                      ...
                      <td>${email}</td>
                      <td>${emailPassword}</td>
                      <td>${2018} Instagram Accounts With Post</td>
                      <td>${price}</td>
                      <td>${da}</td>
                  </tr>`
