...
const fiveYear = (uid, get, child, stt, update, ref, database, nextSibling, input, idNo, current, da, fileLocation, price)=>{
            ...
            get(child(stt, 'users/' + uid)).then( async (snapshot) => {
              ...
              get(child(stt, 'ageAccountsWithPost/' + 2019)).then( async (snapshot) => {
                ...
                if(snapshot.exists()) {
                  ...
                  await update(ref(database, 'ageAccountsWithPost/' + 2019 + '/'),{
                      fiveYear: usd
                })
                  ...
                  for(let l of selectt){
                      ...
                      <td>${email}</td>
                      <td>${emailPassword}</td>
                      <td>${2019} Instagram Accounts With Post</td>
                      <td>${price}</td>
                      <td>${da}</td>
                  </tr>`
