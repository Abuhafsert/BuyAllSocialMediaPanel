import { twoYear } from "./yearsFormatWithPost/twoYears.js";
import { oneYear } from "./yearsFormatWithPost/oneYear.js";
import { threeYear } from "./yearsFormatWithPost/threeYear.js";
import { fourYear } from "./yearsFormatWithPost/fourYears.js";
import { fiveYear } from "./yearsFormatWithPost/fiveYear.js";
import { sixYear } from "./yearsFormatWithPost/sixYears.js";
import { sevenYear } from "./yearsFormatWithPost/sevenYears.js";
import { eightYear } from "./yearsFormatWithPost/eightYears.js";
import { nineYear } from "./yearsFormatWithPost/nineYears.js";
import { tenYear } from "./yearsFormatWithPost/tenYears.js";
import { sleep, dayTime } from "../AllSocialPanel/dayTime.js";
import { oneYearInstagramPriceWithPost,
twoYearInstagramPriceWithPost,
threeYearInstagramPriceWithPost,
fourYearInstagramPriceWithPost,
fiveYearInstagramPriceWithPost,
sixYearInstagramPriceWithPost,
sevenYearInstagramPriceWithPost,
eightYearInstagramPriceWithPost,
nineYearInstagramPriceWithPost,
tenYearInstagramPriceWithPost
...
const accountPricesWithPost = (uid, get, child, stt, update, ref, database)=>{
    btns.forEach(btn => {
        ...
        if(btnValue === 1){
            pricePerAccount = oneYearInstagramPriceWithPost
            prices[0].innerHTML = pricePerAccount * amount[0].value
            onInput(pricePerAccount, btnValue)
            }else if(btnValue === 2){
                pricePerAccount = twoYearInstagramPriceWithPost
                prices[1].innerHTML = pricePerAccount * amount[1].value
                onInput(pricePerAccount, btnValue)
                
            }else if(btnValue === 3){
                pricePerAccount = threeYearInstagramPriceWithPost
                prices[2].innerHTML = pricePerAccount * amount[2].value
                onInput(pricePerAccount, btnValue)
                
                
            }else if(btnValue === 4){
                pricePerAccount = fourYearInstagramPriceWithPost
                prices[3].innerHTML = pricePerAccount * amount[3].value
                
                onInput(pricePerAccount, btnValue)
            }else if(btnValue === 5){
                pricePerAccount = fiveYearInstagramPriceWithPost
...
function quantityWithPost(get, child, stt){
    let array = []
    
    get(child(stt, 'ageAccountsWithPost/' + 2023)).then( async (snapshot) => {
        if(snapshot.exists()) {         
    ...
    get(child(stt, 'ageAccountsWithPost/' + 2022)).then( async (snapshot) => {
        if(snapshot.exists()) {         
    ...
    get(child(stt, 'ageAccountsWithPost/' + 2020)).then( async (snapshot) => {
        if(snapshot.exists()) {         
    ...
    get(child(stt, 'ageAccountsWithPost/' + 2019)).then( async (snapshot) => {
        if(snapshot.exists()) {         
    ...
    get(child(stt, 'ageAccountsWithPost/' + 2018)).then( async (snapshot) => {
        if(snapshot.exists()) {         
...
export { accountPricesWithPost, quantityWithPost }
...
export function ageProduct(uid, get, child, stt, update, ref, database, nextSibling, input, btnValue){
    let idNo = null;
    if(btnValue === 1){
        oneYear(uid, get, child, stt, update, ref, database, nextSibling, input, idNo, current, dayTime, location, oneYearInstagramPriceWithPost)
    } else if(btnValue === 2){
        twoYear(uid, get, child, stt, update, ref, database, nextSibling, input, idNo, current, dayTime, location, twoYearInstagramPriceWithPost)
    }else if(btnValue === 3){           
        threeYear(uid, get, child, stt, update, ref, database, nextSibling, input, idNo, current, dayTime,location, threeYearInstagramPriceWithPost)
    }else if(btnValue === 4){            
        fourYear(uid, get, child, stt, update, ref, database, nextSibling, input, idNo, current, dayTime,location, fourYearInstagramPriceWithPost)
    }else if(btnValue === 5){
        fiveYear(uid, get, child, stt, update, ref, database, nextSibling, input, idNo, current, dayTime,location, fiveYearInstagramPriceWithPost)
    }else if(btnValue === 6){        
