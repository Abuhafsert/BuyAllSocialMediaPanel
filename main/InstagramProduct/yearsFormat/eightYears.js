...
const eightYear = (uid, get, child, stt, update, ref, database, nextSibling, input, idNo, current, da, fileLocation, price)=>{
            ...
            get(child(stt, 'users/' + uid)).then( async (snapshot) => {
              ...
              get(child(stt, 'ageAccounts/' + 2016)).then( async (snapshot) => {
                ...
                if(snapshot.exists()) {
                  ...
                  await update(ref(database, 'ageAccounts/' + 2016 + '/'),{
                      eightYear: usd
