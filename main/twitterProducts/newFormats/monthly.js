import { balance } from "../twitternew.js"
    ...
    await get(child(stt, 'users/' + uid )).then(async(snapshot) => {
          ...
          if(parseFloat(nextSibling) > Number(userBalances)){
            ...
            await get(child(stt, 'users/' + uid)).then( async (snapshot) => {
              ...
              await get(child(stt, 'monthlyTwitterAccounts/')).then( async (snapshot) => {
                ...
                if(snapshot.exists()) {
                  ...
                  if(input > usd.length){
                    ...
                    await update(ref(database, 'monthlyTwitterAccounts/'),{
                        monthly: usd
                  ...
                  for(let l of selectt){
                      ...
                      <td>${email}</td>
                      <td>${emailPassword}</td>
                      <td>Monthky Twitter Accounts</td>
                      <td>${price}</td>
                      <td>${da}</td>
                  </tr>`
