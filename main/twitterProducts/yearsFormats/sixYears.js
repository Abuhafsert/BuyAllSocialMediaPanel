...
const sixYear = async (uid, get, child, stt, update, ref, database, nextSibling, input, idNo, current, da, fileLocation, price)=>{
            ...
            await get(child(stt, 'users/' + uid)).then( async (snapshot) => {
              ...
              await get(child(stt, 'ageTwitterAccounts/' + 2018)).then( async (snapshot) => {
                ...
                if(snapshot.exists()) {
                  ...
                  await update(ref(database, 'ageTwitterAccounts/' + 2018 + '/'),{
                      sixYear: usd
