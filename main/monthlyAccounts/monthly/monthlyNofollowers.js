import { balance } from "../monthlyInstagram.js"
import { sleep } from "../../AllSocialPanel/dayTime.js"



function appendChildToBody(selectt, idNo, tbod){
  ...
  for(let l of selectt){
    ...
    idNo++;
      ...
      <td>${email}</td>
      <td>${emailPassword}</td>
      <td>monthly Instagram no Followers</td>
      <td>${price}</td>
      <td>${da}</td>
  </tr>`
...
const monthly = async (uid, get, child, stt, update, ref, database, nextSibling, input, idNo, current, da, fileLocation, price)=>{
            ...
            get(child(stt, 'users/' + uid)).then(async (snapshot) => {
              ...
              get(child(stt, 'monthlyinstaNoFollowers/')).then( async (snapshot) => {
                ...
                if(snapshot.exists()) {
                  ...
                  await update(ref(database, 'monthlyinstaNoFollowers/'),{
                    new: usd
                  })

                  // await appendChildToBody(selectt, idNo, tbod)
