import { monthly } from "./monthly/monthlyNofollowers.js"
import { sleep, dayTime } from "../AllSocialPanel/dayTime.js"
import { referralInstagramPriceWithPost } from "../pricesAccounts/priceAccount.js"
...
function monthlyProductNoFollowers(uid, get, child, stt, update, ref, database){
    ...
    if(btnValue === 1){
        price = referralInstagramPriceWithPost
...
function monthlyinstaNoFollowers(get, child, stt){
    let array = []
    
    get(child(stt, 'monthlyinstaNoFollowers/')).then( async (snapshot) => {
        if(snapshot.exists()) {         
...
export { monthlyinstaNoFollowers }        
...
function ageProduct(uid, get, child, stt, update, ref, database, nextSibling, input, btnValue){
    let idNo = null;
    if(btnValue === 1){
        monthly(uid, get, child, stt, update, ref, database, nextSibling, input, idNo, current, dayTime, location, referralInstagramPriceWithPost)
    } 
}
      
      
      
export { monthlyProductNoFollowers }
